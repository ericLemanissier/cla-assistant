...
const commentText = (signed, badgeUrl, claUrl, userMap, recheckUrl, migrationNotice) => {
    ...
    if (committersCount > 1) {
        ...
        userMap.signed.toSorted().forEach(function (signee) {
            text += '<br/>:white_check_mark: ' + signee
        })
        userMap.not_signed.toSorted().forEach(function (signee) {
            text += '<br/>:x: ' + signee
        })
        text += '<br/>'
    }
    ...
    if (userMap && userMap.unknown && userMap.unknown.length > 0) {
        ...
        text += `<hr/>**${userMap.unknown.toSorted().join(', ')}** ${seem} not to be a GitHub user.`
